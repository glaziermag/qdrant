...
use super::{BLOCK_SIZE, BlockKey, BlockOffset, BlockRequest, FileId};
...
pub struct CacheController {
    ...
    pub(super) block_id_counter: Mutex<BlockKey>,

    pub(super) cache:
        trififo::ShardedCache<BlockKey, BlockOffset, BlocksLifecycle, ahash::RandomState>,
...
impl CacheController {
    ...
    pub fn new(cache_path: &Path, size_bytes: u64) -> io::Result<Arc<CacheController>> {
        ...
        Ok(Arc::new(CacheController {
            ...
            block_id_counter: Mutex::new(BlockKey(0)),
            cache,
            blocks_lifecycle,
            cache_file,
            cache_mmap,
    ...
    pub fn open_file(self: &Arc<Self>, path: &Path) -> io::Result<CachedFile> {
        ...
        let starting_block_id = {
            ...
            *file_id_counter = BlockKey(file_id_counter.0.checked_add(num_blocks).unwrap_or_else(
                || {
        ...
        let file_id = FileId {
            starting_block: starting_block_id,
        };
    ...
    pub(super) fn get_from_cache(&self, req: BlockRequest) -> io::Result<Cow<'_, [u8]>> {
        ...
        match self.cache.get_or_guard(&key) {
            ...
            GetOrGuard::Guard(guard) => {
                ...
                let offset = (key.0 - file_id.starting_block.0) * BLOCK_SIZE as u64;
...
impl quick_cache::Lifecycle<BlockKey, BlockOffset> for BlocksLifecycle {
    type RequestState = Option<(BlockKey, BlockOffset)>;

    fn begin_request(&self) -> Self::RequestState {
        None
    }

    fn on_evict(&self, state: &mut Self::RequestState, key: BlockKey, val: BlockOffset) {
    ...
    fn is_pinned(&self, _key: &BlockKey, _val: &BlockOffset) -> bool {
...
impl trififo::lifecycle::Lifecycle<BlockKey, BlockOffset> for BlocksLifecycle {
    fn on_evict(&self, _key: BlockKey, value: BlockOffset) {
