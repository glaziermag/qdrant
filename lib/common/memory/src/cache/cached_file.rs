...
use super::{BLOCK_SIZE, BlockKey, BlockRequest, CacheController};
...
fn blocks_for_range_in(
    ...
    (first_block..last_block + 1).map(move |block| {
        let block_key = BlockKey(file_id.starting_block.0 + block as u64);

        let range_start = if block == first_block {
            leading_offset
        } else {
            0
        };

        let range_end = if block == last_block {
            trailing_offset
        } else {
            BLOCK_SIZE
        };
